...
def main():
    ...
    with container4:
        st.header("Point_Spread and blur matrix")
        ksize = selected_image.kernel_size
        psf = selected_image.psf(sigma_L=np.array([selected_image.sigma_blur, selected_image.sigma_blur]), p_size=ksize[0] ) 
        h_f = sp.fft.fftn(psf.reshape(psf.size))

        fig = plt.figure()

        plt.scatter( x=np.angle(h_f), y=np.abs(h_f))
        ...
        r1.show_array(r1.image, title= " Original 10 by 10 image")
        st.write(r1.shape)


        image_shape = r1.shape[0:2]
        image_size = r1.shape[0] * r1.shape[1]
        k_matrix = Image_Processing.blur_matrix(psf, image_shape=image_shape)
        st.write(k_matrix)

        example1 = np.dot(k_matrix, r1.image.reshape(image_size) )

        Photo.show_array(example1.reshape(image_shape), title="Example Image using blur matrix k")

        st.header("Try to get the SVD of k matrix then")
        n_components = 15
        from scipy.sparse.linalg import svds
        U, s, Vh = svds(k_matrix, k=n_components) # Compute the truncated svd out to 6 components
        st.write( U.shape, s.shape, Vh.shape)
        st.write((np.diag(s) ) )

        y = example1.reshape(image_size)
        x = (np.transpose(Vh)@ (np.diag(1/s) ) @ (np.transpose(U) @ y) ).reshape(image_shape)

        Photo.show_array( x, title="Truncated SVD to De-Blur image... truncated after {n_components} components....")
        ...
        st.header(" Testing Newtons method for adjusting the psuedo inverse")
        n_components = 15
        U, s, Vh = svds(k_matrix, k=n_components) # Compute the truncated svd out to 6 components

        '''
        The goal is to Compute an adjusted psudo inverse that takes my constraintes into account.
        Namely that the noise is assumed to be gaussian
        '''
        x = r1.image.reshape(image_size) # Original Image   
        y = example1.reshape(image_size) # Blurred Image using matrix K

        Photo.show_array( y.reshape(image_shape), title="Blur image... ")
        # step 1 calculate initial x_0
        x_0 = (np.transpose(Vh)@ (np.diag(1/s) ) @ (np.transpose(U) @ y) ).reshape(image_shape)
        Photo.show_array(x_0.reshape(image_shape), title="First approximation of deblurred image... Percent Error is  {}%".format( 100*np.norm(x - x_0)/np.norm(x) ))
        i = 1
        TOL = np.pow(10,-3)
        error_0 = x - x_0
        s2_0 = s
        sigma_blur = selected_image.sigma_blur
        ss = sigma_blur*sigma_blur
        while np.norm(error_0) < TOL:
            rns = 1/error_0 # recipricol noise vector
            cost = np.sum(error_0*error_0/(s2_0*ss))
            s2_0 = s2_0 * ( np.ones(s2_0) - ss*rns*rns*cost*s2_0 )
            x_0 = (np.transpose(Vh)@ (np.diag(1/s) ) @ (np.transpose(U) @ y) ).reshape(image_shape)
            error_0 =  x - x_0
            if i % 10 == 0:
                Photo.show_array(x_0.reshape(image_shape), title="Approximation of unblurred image x, Iteration {i}, Percent Error {}".format(i, error_0))
            i+=1
